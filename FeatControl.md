...
Se usan los **14 alérgenos de declaración obligatoria en la UE** (Reglamento 1169/2011) como casillas, más **nueces** por separado (legalmente van dentro de "frutos de cáscara", pero hay quien solo es alérgico a ellas), y un campo libre opcional de observaciones. Un test comprueba que la lista de `allergens.ts`, el enum `Allergen` de GraphQL y las traducciones coinciden, porque añadir un alérgeno en un solo sitio rompe el guardado. La lista cerrada permite traducir, agrupar y resumir, y evita que "celiaco", "sin gluten" y "gluten" sean tres cosas distintas. En el perfil propio aparece como un panel plegable (ANTD `Collapse`) con el resumen en la cabecera, para no empujar hacia abajo las estadísticas.
